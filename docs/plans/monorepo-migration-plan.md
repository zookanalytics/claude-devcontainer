...
| **claude-instance** | `signal-loom/scripts/claude-instance` | Multi-instance management (~500 lines bash) |
| **bmad-orchestrator** | `_repos/bmad_orchestrator/` | BMAD workflow orchestration (Python CLI + hooks) |
| **project hooks** | `signal-loom/.claude/hooks/` | Commit enforcement, session focus, etc. |
...
### Current Skill-Hook Bundles in signal-loom
...
│    → No commits needed in signal-loom!                          │
└─────────────────────────────────────────────────────────────────┘
```
...
2. Copy `signal-loom/scripts/claude-instance` to `packages/claude-instance/bin/`
...
2. Move git-related skills from signal-loom:
...
### Phase 6: Update signal-loom (consumer)
...
- [ ] No tool-related code in signal-loom (only config)
